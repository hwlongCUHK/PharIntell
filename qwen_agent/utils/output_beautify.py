    ...
    for msg in messages:
        if msg['role'] == ASSISTANT:
            if msg.get('reasoning_content'):
                ...
                content.append(f'{Fore.CYAN + Style.BRIGHT}{THOUGHT_S}\n{msg["reasoning_content"]}{Style.RESET_ALL}')
            if msg.get('content'):
                assert isinstance(msg['content'], str), 'Now only supports text messages'
                content.append(f'{Fore.CYAN + Style.BRIGHT}{ANSWER_S}\n{msg["content"]}{Style.RESET_ALL}')
            if msg.get('function_call'):
                content.append(f'{Fore.CYAN + Style.BRIGHT}{TOOL_CALL_S} {msg["function_call"]["name"]}\n{msg["function_call"]["arguments"]}{Style.RESET_ALL}')
        elif msg['role'] == FUNCTION:
            content.append(f'{Fore.GREEN + Style.BRIGHT}{TOOL_RESULT_S} {msg["name"]}\n{msg["content"]}{Style.RESET_ALL}')
        else:
            raise TypeError
    if content:
        full_text = '\n'.join(content)
