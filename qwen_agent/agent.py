...
class Agent(ABC):
    ...
    def run(self, messages: List[Union[Dict, Message]],
        ...
        for rsp in self._run(messages=new_messages, **kwargs):
            if type(rsp) == list:
                ...
                if not rsp.name and self.name:
                    rsp.name = self.name
                if _return_message_type == 'message':
                    yield [Message(**rsp) if isinstance(rsp, dict) else rsp]
                else:
                    yield [rsp.model_dump() if not isinstance(rsp, dict) else rsp]
    ...
    def find_func_call(text):
        ...
        patterns = [
            ...
            r'<tools>(.*?)</tools>', 
            r'<tool>(.*?)</tool>', 
            r'\[TOOL_CALLS\]tool_call>(.*?)</tool_call>',
            r'^([A-Za-z0-9_]+)\n({.*?})(?=\w*\n|$)'
            ]
        matched = False
        for pattern in patterns:
            matches = re.findall(pattern, cleaned_text.strip(), re.DOTALL)
            for match in matches:
                matched = True
                if pattern == r'^([A-Za-z0-9_]+)\n({.*?})(?=\w*\n|$)':
                    try:
                        func_call = {"name": match[0], "arguments": json5.loads(match[1])}
                        yield json.dumps(func_call)
                    except ValueError:
                        continue
                else:
                    yield match.strip()
    ...
    def _detect_tool(self, message: Message) -> Tuple[bool, str, str, str]:
        ...
        if message.function_call:
            ...
            yield (func_name is not None), func_name, func_args, text
        else:
            for func_call in self.find_func_call(message.content):
                try:
                    func_call = json5.loads(func_call)
                    if 'type' in func_call and func_call['type'] == 'function':
                        func_call = func_call['function']
                    func_name = func_call["name"]
                    func_args = json5.dumps(func_call["arguments"], ensure_ascii=False, indent=4)
                    use_tool = True
                    yield True, func_name, func_args, text
                except:
                    func_call = None
                    func_name = None
                    func_args = None
            if not use_tool:
                yield False, func_name, func_args, text
